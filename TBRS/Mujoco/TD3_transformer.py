...
mconf = GPTConfig(4, 3,
            n_layer=6, n_head=8, n_embd=128, model_type='reward_conditioned', max_timestep=999)
model = GPT(mconf)
model.load_state_dict(torch.load('RT_hopper'))
...
if __name__ == '__main__':
    ...
    for repeat in range(args.repeat, args.repeat+1):
        ...
        for t in range(int(args_max_timesteps)):
            ...
            states= np.array([np.array([state])])
            acts = np.reshape([action],(1,-1,3))
            states = torch.tensor(states, dtype=torch.float32)
            acts = torch.tensor(acts, dtype=torch.float32)
            
            timesteps = np.arange(episode_timesteps-1, episode_timesteps, 1)
            timesteps = np.reshape(timesteps,(1,-1,1))
            timesteps = torch.tensor(timesteps, dtype=torch.int64)
            states = states.to(device)
            timesteps = timesteps.to(device)
            acts = acts.to(device)
            if t+1 < args_start_timesteps:
                next_action = np.random.uniform(-max_action, max_action, action_dim)
            else:
                if np.random.uniform(0,1) < 0.0:
                    next_action = np.random.uniform(-max_action, max_action, action_dim)
                else:
                    next_action = (
                        policy.select_action(np.array(next_state))
                        + np.random.normal(0, max_action * args_expl_noise, size=action_dim)
                    ).clip(-max_action, max_action)
            next_states = np.array([np.array([next_state])])
            next_acts = np.reshape([next_action],(1,-1,3))
            next_states = torch.tensor(next_states, dtype=torch.float32)
            next_acts = torch.tensor(next_acts, dtype=torch.float32)
            next_timesteps = np.arange(episode_timesteps, episode_timesteps+1, 1)
            next_timesteps = np.reshape(next_timesteps,(1,-1,1))
            next_timesteps = torch.tensor(next_timesteps, dtype=torch.int64)
            next_states = next_states.to(device)
            next_timesteps = next_timesteps.to(device)
            next_acts = next_acts.to(device)
            ir,_ = model(states, acts, None, None, timesteps)
            next_ir,_ = model(next_states, next_acts, None, None, next_timesteps)
            phi=ir.cpu().detach().numpy()
            next_phi=next_ir.cpu().detach().numpy()
            intrinsic_reward = (next_phi[0][0][0]*0.8 - phi[0][0][0])/100.0



            done_bool = float(done) if episode_timesteps < 1000 else 0
            replay_buffer.add(state, action, next_state, reward+intrinsic_reward, done_bool)
            state = next_state
            ...
            if done or episode_timesteps > 998:
